...
- `@urateam/cli` — `ura dev` for local development, `ura start` for production. User-level surface: `ura init` / `ura repo {add,list,remove}` / `ura uninstall` / `ura service {install,uninstall}` (auto-start via launchd or systemd-user).
