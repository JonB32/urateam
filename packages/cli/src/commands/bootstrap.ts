...
/** Default app (webhook server) port. */
...
export interface BootstrapContext {
  ...
  /** App / webhook server host port (default 3000). */
  appPort?: number;
  /** Dashboard host port (default 3001). */
 ...
 * @param ports - Ports to check for availability. Defaults to [3000, 3001].
 */
export async function preflightChecks(
  deps?: BootstrapDeps,
  ports: readonly number[] = [DEFAULT_APP_PORT, DEFAULT_DASHBOARD_PORT],
): Promise<void> {
  // --- Docker ----------------------------------------------------------------
  try {
  ...
  const portResults = await Promise.all(ports.map((p) => portCheck(p)));
  const busyPortIdx = portResults.findIndex((free) => !free);
  if (busyPortIdx !== -1) {
    const port = ports[busyPortIdx];
    throw new Error(
 ...
 * Host ports are taken from `ctx.appPort` / `ctx.dashboardPort` (defaults: 3000/3001).
 *
 ...
 * @param deps      - Optional injectable dependencies (for testing).
 * @param appPort   - App / webhook server host port (default 3000).
 */
export async function generateReverseProxyConfig(
  ...
  if (choice === "caddy") {
    ...
    log(`To expose port ${appPort} via Cloudflare Tunnel, run:`);
    log(`  cloudflared tunnel --url http://localhost:${appPort}`);
    log(
...
export const bootstrapCommand = new Command("bootstrap")
  ...
  .action(async (opts: {
    ...
    // ── Resolve ports ──────────────────────────────────────────────────────
    // --port is a deprecated alias for --app-port.
    if (opts.port !== undefined) {
      logger.warn(
        "--port is deprecated. Use --app-port instead. " +
    ...
    try {
      await preflightChecks(undefined, [resolvedAppPort, resolvedDashboardPort]);
    ...
    // ── Step 7: Validation ─────────────────────────────────────────────────
    if (opts.validate) {
      logger.info({ port: resolvedAppPort }, "[7/7] Validating stack — waiting up to 30s for webhook server...");
      try {
        await validateSetup(resolvedAppPort);
