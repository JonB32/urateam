...
describe("addMRComment", () => {
  ...
  it("posts a comment to the correct notes endpoint", async () => {
    const fetchSpy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(makeResponse({ id: 1, body: "Hello" }, 201));

    await addMRComment(config, "myorg/myrepo", 7, "Hello from the agent");

    const [url, init] = fetchSpy.mock.calls[0] as [string, RequestInit];
    expect(url).toBe(
      "https://gitlab.com/api/v4/projects/myorg%2Fmyrepo/merge_requests/7/notes",
    );
    expect(init.method).toBe("POST");
    expect((init.headers as Record<string, string>)["PRIVATE-TOKEN"]).toBe(
      "glpat-test",
    ...
    const body = JSON.parse(init.body as string);
    expect(body.body).toBe("Hello from the agent");
  });

  it("resolves without error on a 200 response", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
      makeResponse({ id: 2, body: "ok" }, 200),
    );

    await expect(
      addMRComment(config, "myorg/myrepo", 1, "test"),
  ...
  it("throws a descriptive error on non-ok response", async () => {
    ...
    await expect(
      addMRComment(config, "myorg/myrepo", 99, "comment"),
    ).rejects.toThrow("GitLab API error 404 adding MR comment");
  });

  it("includes the API response body text in the thrown error", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
      makeResponse("MR not found", 404, { ok: false }),
    );
  ...
  it("uses a custom self-hosted host in the notes URL", async () => {
    const selfHostedConfig: GitLabConfig = {
      token: "mytoken",
      host: "https://gitlab.example.com",
    };
    ...
    const fetchSpy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(makeResponse({ id: 3, body: "ok" }, 201));

    await addMRComment(selfHostedConfig, "myorg/myrepo", 5, "a comment");

    const [url] = fetchSpy.mock.calls[0] as [string];
    expect(url).toBe(
      "https://gitlab.example.com/api/v4/projects/myorg%2Fmyrepo/merge_requests/5/notes",
    );
  });

  it("percent-encodes project paths with nested groups", async () => {
    const fetchSpy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(makeResponse({ id: 4, body: "ok" }, 201));

    await addMRComment(config, "org/sub/project", 2, "nested group comment");

    const [url] = fetchSpy.mock.calls[0] as [string];
    expect(url).toContain("org%2Fsub%2Fproject");
  });
  ...
  it("throws on a 401 Unauthorized response", async () => {
    ...
    await expect(
      addMRComment(config, "myorg/myrepo", 1, "secret"),
    ).rejects.toThrow("GitLab API error 401 adding MR comment");
  });
});
