...
/**
 * The resolved Claude authentication method and credentials.
...
export interface ClaudeAuthCredentials {
  ...
  /**
   * The OAuth token value when method === "oauth-token". The Agent SDK reads
   * CLAUDE_CODE_OAUTH_TOKEN from the environment automatically, so callers
   * rarely need to use this field directly; it is surfaced here for logging
   * and explicit SDK initialisation if the SDK ever requires it.
   */
  oauthToken?: string;
}
