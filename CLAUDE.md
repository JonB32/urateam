...
- **Deep review + convergence detection (BEC-208)**: The deep-review loop (Enterprise `deep-review` feature) runs 3 parallel sub-agents (reuse, quality, efficiency) per pass. Two convergence guards prevent runaway loops:
  1. **Turn limit** (`maxReviewTurns`, default 12): the loop hard-stops after this many iterations and forces the PR to draft with a `convergence-failure` blocking finding.
  2. **Repeated-issue detector**: if the same unresolved issues appear in **3 consecutive turns** (configurable via `consecutiveThreshold` in `convergenceValidator.ts`), the loop stops early. Sub-case diagnosis:
     - `"non-responsive-implementation"`: the implementation diff hash did not change — the agent is ignoring the feedback.
     - `"misaligned-review-criteria"`: the implementation changed but the same issues were re-raised — review criteria may be ambiguous or too strict.
  - Module: `packages/core/src/pipeline/convergenceValidator.ts` — `checkConvergence(history, config)` is pure and called after each deep-review turn in `runner.ts`.
  - Config field: `maxReviewTurns` (integer ≥ 1, optional, default 12) in `PipelineConfig`. Pair with `deepReviewPasses` to control the loop depth.
  - The implementation diff hash is computed via `git diff HEAD | sha256 | first-16-chars` after each implement re-run and embedded in the `buildDeepReviewContext` output so downstream tools can inspect it.
