...
- **Multi-AI provider support (BEC-201)**: implement stage supports non-Anthropic AI backends via the `AgentProvider` abstraction in `executor/provider/`. Provider selection (highest → lowest precedence): `IMPLEMENT_PROVIDER` env var > `pipelineConfig.stageProviders.implement` > default `"anthropic-sdk"`. Supported providers:
  - `"anthropic-sdk"` (default) — `@anthropic-ai/claude-agent-sdk`, agentic/tool-using, full file-system access.
  - `"openrouter"` — OpenRouter chat-completion API (200+ models: Claude, GPT, Gemini, Kimi…). Requires `OPENROUTER_API_KEY`. Model set via `stageModels.implement` or `IMPLEMENT_OPENROUTER_MODEL` env var. **Limitation**: one-shot (no tools), model produces a plan/handoff but cannot write files directly.
  - Fallback behavior: unknown provider IDs fall back to `"anthropic-sdk"` with a warn log. API errors throw a descriptive `"OpenRouterAgent: provider call failed for model X: ..."` error, which executor records as a failed stage.
  - `stage_runs.provider_name` and `stage_runs.model_id` columns track actual provider/model used per stage (set even on failure).
