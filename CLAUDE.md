...
- **Auto-deep-review thresholds** (Tier 3, `packages/core/src/pipeline/auto-deep-review.ts`): promotes the deep-review fanout from opt-in to default-on for non-trivial PRs. After the review-fix loop, the runner computes diff metrics (`newFiles`, `totalLines`, `newPublicExports` — the last via `countNewPublicExports` parsing `+export (function|class|const|let|interface|type|enum) ...` lines under `packages/<pkg>/src/`, excluding `__tests__/`). If any metric trips `autoDeepReviewThresholds` (defaults `{ newFiles: 5, totalLines: 200, newPublicExports: 2 }`) AND `OPENROUTER_API_KEY` + `REVIEW_MODELS` are set AND the `deep-review` license is active, `deepReviewPasses` is bumped to `max(deepReviewPasses, 1)`. Operators can disable globally via `URATEAM_DISABLE_AUTO_DEEP_REVIEW=true` or per-pipeline via `autoDeepReviewThresholds: { newFiles: 999999, ... }`. New `deepReviewFindingsAreBlocking` (default `true`) upgrades every deep-review finding's severity to `"blocking"` so it forces draft via the existing `unresolvedBlockingFindings` path; set to `false` to restore the pre-Tier-3 advisory behavior. No new audit event (the existing deep-review logs cover this).
