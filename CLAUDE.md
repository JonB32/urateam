...
- **Stalled stage detection (BEC-210):** `checkStalledStages` (`actions/check-stalled-stages.ts`) runs on every PM tick and queries `active_work` for entries where `updatedAt < now - threshold`. Default threshold is **30 minutes** — override via `PM_AGENT_STALLED_STAGE_THRESHOLD_MIN` env var (min 1), or the `stalledStageThresholdMinutes` field in `PmAgentConfig`. When stalled runs are found, a structured `warn` log is emitted per run with keys: `runId`, `stageName`, `lastActiveTimestamp`, `stalledDurationSeconds`. This complements the in-executor `StageStalledError`/`StagePreStreamStalledError` (BEC-183) defences — those fire from inside the agent stream, while this catches hangs that survive past the executor (e.g. orphaned `active_work` rows after a process restart). **Manual recovery:** `POST /runs/:id/resume-stalled` (dashboard) marks the run as `retriable` and removes the stale `active_work` row; the next PM tick's `recoverRetriableRuns` sweep re-queues it. **Troubleshooting:** if a run shows as stalled but is actually still making progress, check if `active_work.updatedAt` is being updated by `upsertActiveWork` — a bug in the executor's coordination calls can cause false-positive stall alerts.
