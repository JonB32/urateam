...
- **Review loop convergence (BEC-213):** Both the review-fix loop and deep-review loop share a `reviewTurns` counter and terminate when any of three conditions hold: **(a)** findings are empty, **(b)** current findings are content-identical to previous cycle (`isConverged()` in `pipeline/convergence-checker.ts` — compares `file+line+category+description` fingerprints, NOT just counts), or **(c)** `reviewTurns >= maxReviewTurns` (`PipelineConfig.maxReviewTurns`, default `MAX_REVIEW_TURNS = 15`). On condition (c) a `buildMisalignmentReport()` is logged at WARN with each cycle's findings and diff to aid diagnosis. `LOOP_TURN_THRESHOLD` in `packages/observers/src/run-patterns.ts` is 15 so the quality observer flags any run exceeding this value. **Stalled pipeline diagnosis**: search logs for `"maxReviewTurns reached"` and inspect the embedded misalignment report — look for findings that recur across cycles unchanged (implement not addressing review's concerns) or diffs that don't touch the files flagged by review.
...
- 17 event types: `run.{started,completed,failed,auto_merged,auto_merge_skipped}`, `pm.{approval_requested,approval_resolved,issue_promoted,issue_deprioritized,issue_cancelled,triage_classified}`, `budget.{alert_fired,run_refused}`, `license.validation_failed`, `config.loaded`, `dashboard.manual_action` (reserved), `claude.auth_expired` (BEC-207, AuthMonitor)
