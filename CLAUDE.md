...
- `packages/cli/` — CLI entry point. **User-level install path** (`ura init` → `ura repo add` → `ura start`) reads `~/.urateam/config.json` via `buildRepoConfigsFromEnv` when no `REPO_*` env vars are set. See `deploy/USER_LEVEL_INSTALL.md`. Project-level (sidecar) install stays env-var-driven for back-compat. **Service auto-start** (`ura service install` / `ura service uninstall`, `packages/cli/src/commands/service.ts`): generates a launchd plist (macOS) or systemd-user unit (Linux) so the daemon auto-starts on login. Pure I/O-free unit-file generators live in `packages/cli/src/lib/service-unit.ts` (`renderLaunchdPlist`, `renderSystemdUserUnit`); the command writes them to `~/Library/LaunchAgents/com.urateam.daemon.plist` or `~/.config/systemd/user/urateam.service` and invokes `launchctl load -w` / `systemctl --user daemon-reload + enable --now`. Idempotent (refuses to overwrite — uninstall first). `--dry-run` prints the unit content without filesystem mutation. Audit events `service.installed` / `service.uninstalled` are emitted opportunistically (only when the daemon SQLite already exists, since CLI runs can predate `ura start`). Unsupported platforms (Windows, BSD) fail with a clear pointer to the manual snippets in `deploy/USER_LEVEL_INSTALL.md`. **Linear OAuth** (`ura self-auth-linear`, `packages/cli/src/commands/self-auth-linear.ts`): browser-based OAuth flow. Preconditions: operator ran `ura init` and set `LINEAR_CLIENT_ID` / `LINEAR_CLIENT_SECRET` in `~/.urateam/.env` (Linear OAuth apps created at `https://linear.app/settings/api/applications`). The CLI spins up an ephemeral `127.0.0.1:<random-port>` server, opens the authorize URL via `open` (macOS) / `xdg-open` (Linux) / printed-URL fallback, verifies the HMAC-signed `state` (per-invocation 32-byte random secret; never persisted; timing-safe compare via `node:crypto.timingSafeEqual`), exchanges the code at `https://api.linear.app/oauth/token`, fetches workspace metadata via Linear GraphQL, and writes `LINEAR_API_KEY=<token>` to `~/.urateam/.env` via atomic `upsertEnvFile()` (rename-after-write; preserves unrelated keys / comments / blanks). Audit event `linear.oauth_completed` is emitted opportunistically; payload carries workspaceId + workspaceName, never the token. Tokens NEVER appear in console output or the success-page HTML. Flags: `--timeout-ms` (default 5min, used to abort if the operator never authorizes), `--scope` (default `read,write`), `--port` (default 9898 — Linear requires the redirect URI to match the registered one exactly, so the port is fixed; pass a different value if 9898 collides, and register the matching URI in Linear). Surfaces `EADDRINUSE` with a clear "port already in use" error. Webhook-secret setup remains manual — Linear's API doesn't expose existing webhook secrets to OAuth-authorized callers. **Tunnel manager** (`packages/cli/src/lib/tunnel.ts`, wired in `ura start` via `--tunnel <mode>`): supervises `cloudflared` as a child process so operators don't have to run it in a separate terminal. Three modes: `none` (default — daemon stays on local ports), `cloudflare-quick` (free, ephemeral `*.trycloudflare.com` URL; parsed from cloudflared's stderr), `cloudflare-token` (named tunnel; requires `CLOUDFLARE_TUNNEL_TOKEN` + `URATEAM_PUBLIC_URL` env vars — the URL must be known ahead of time since Cloudflare doesn't print it back). Exponential-backoff restart (1s → 30s capped, 10 attempts max) on unexpected exit; SIGTERM on `stop()` from the graceful shutdown path. On startup the public URL is exported as `process.env.URATEAM_PUBLIC_URL` so OAuth callbacks, webhook signing, etc. see it. When `cloudflared` isn't on `$PATH` (`ENOENT` on spawn), `CloudflaredMissingError` is raised with brew/apt/release-page install hints. Audit events `tunnel.started` (provider, publicUrl, restartCount) and `tunnel.stopped` (provider, restartCount, exitCode, signal) emit through license-gated `logAuditEvent`. Failure to start the tunnel never crashes the daemon — operators can still reach the local ports for debugging. **Config hot-reload** (`packages/cli/src/lib/config-watcher.ts`, wired in `ura start` when no `REPO_*` env vars are set AND `~/.urateam/config.json` exists): watches the user-level config via `fs.watch` with debouncing (1s default), re-parses + zod-validates on change, diffs against the previous in-memory config. **Safe field changes** (`labelPattern`, `testCommand`, `buildCommand`, `teamId`) apply immediately by mutating the live `repoConfigs` object. **Unsafe field changes** (`url`, `path`, `defaultBranch`) log a `restart required` warning and don't mutate runtime state. **Add**: new entry inserted live; webhook routes via `selectRepoConfig` see the new repo on the next inbound event. **Remove**: entry deleted from `repoConfigs` immediately — in-flight pipeline runs hold their own snapshot of state and continue uninterrupted, so removal only affects routing of NEW work (operators wanting to forcibly stop in-flight runs use `ura stop` / `ura halt`). Schema-validation failures keep the previous in-memory config and log a warning. Audit event `config.reloaded` (added, removed, modifiedSafe, modifiedUnsafe arrays + sha256 hash of the new config) emits per applied reload via `logAuditEvent`. Project-level (sidecar) installs don't use this — the watcher only activates in user-level mode.
...
- **Typecheck gate** (Tier 1b, `packages/core/src/pipeline/typecheck-gate.ts`): runs `pnpm -w typecheck` (configurable) inside the worktree right after the scratch-file gate, before the push queue. On non-zero exit: pushes a single `category: "typecheck"` blocking `ReviewFinding` (description includes the first 5 error lines, fix points operators at the escape hatch + the PR-body output), forces `shouldDraft = true`, and emits a `pipeline.typecheck_failed` audit event (payload: `errorCount`, `firstMessages` capped at 5 lines × 500 chars). Output is truncated at 50 KB. Escape hatch: `URATEAM_DISABLE_TYPECHECK_GATE=true`. Fail-open on runner error (logged, swallowed). The gate is unit-tested via a `TypecheckRunner` DI hook so the test suite doesn't need a real `pnpm`/`tsc` install.
- **Spec-vs-impl JSDoc gate** (Tier 1c, `packages/core/src/pipeline/spec-vs-impl-gate.ts`): scans the agent's added/modified `.ts`/`.tsx`/`.js`/`.jsx` files for JSDoc references matching `\b(config|opts|env|deps|options)\.([A-Za-z_][A-Za-z0-9_]*)\b` inside `/** ... */` blocks, then checks whether each bare symbol appears anywhere in the worktree's tracked source via a single `.includes()` over a concatenated corpus. On unresolved references: pushes one `category: "spec-vs-impl"` blocking finding per (file, symbol), forces draft, emits `pipeline.spec_vs_impl_failed` audit event (payload caps findings at 20). Catches the PR #254 BEC-201 failure mode where docs promised `config.implementProviderFallback` but the field was never added to the Zod schema. Heuristic — false positives are accepted per the operator brief; the corpus check is intentionally loose (substring match, not full AST). Escape hatch: `URATEAM_DISABLE_SPEC_VS_IMPL_GATE=true`. Unit-tested via `checkSpecVsImplFromContent` with an injectable `corpusContainsSymbol` so no worktree or git needed in tests.
- **Project-convention review prompt** (Tier 2, `packages/core/src/security/review-checklist.ts:PROJECT_CONVENTION_CHECKLIST`): injects a 9-category checklist into the review-stage prompt. The agent emits `ReviewFinding`s using these `category` strings verbatim — same vocabulary as the deterministic gates so operators see one consistent set across both surfaces. Categories: `scratch-files`, `db-ddl-drift`, `audit-bypass-undocumented`, `credential-in-interface`, `spec-vs-impl`, `convention-execfile`, `convention-console`, `convention-throw`, `convention-as-any`. Review stage defaults to `claude-sonnet-4-6` (same as implement; never Haiku for review) via `DEFAULT_AGENT_PROFILES.review.model`. Categories also enumerated in `REVIEW_OUTPUT_FORMAT` so the structured-output parser accepts them. The review-fix loop already treats blocking findings as a re-implement trigger; no runner-side changes needed for this tier.
- **Auto-deep-review thresholds** (Tier 3, `packages/core/src/pipeline/auto-deep-review.ts`): promotes the deep-review fanout from opt-in to default-on for non-trivial PRs. After the review-fix loop, the runner computes diff metrics (`changedFiles` — git diff --stat's "N files changed" line; `totalLines` — insertions+deletions; `newPublicExports` — via `countNewPublicExports` parsing `+export (default ?)(async ?)(function|class|const|let|interface|type|enum|{|*) ...` lines under `packages/<pkg>/src/` OR repo-root `src/`, excluding `__tests__/`). If any metric trips `autoDeepReviewThresholds` (defaults `{ changedFiles: 5, totalLines: 200, newPublicExports: 2 }`) AND the `deep-review` license is active, `deepReviewPasses` is bumped to `max(deepReviewPasses, 1)`. The agentic deep-review provider runs on Claude regardless of OpenRouter env vars; fanout providers are an additional layer that activates when `OPENROUTER_API_KEY` + `REVIEW_MODELS` are set. Operators can disable globally via `URATEAM_DISABLE_AUTO_DEEP_REVIEW=true` or per-pipeline via `autoDeepReviewThresholds: { changedFiles: 999999, ... }`. New `deepReviewFindingsAreBlocking` (default `true`) upgrades every deep-review finding's severity to `"blocking"` so it forces draft via the existing `unresolvedBlockingFindings` path; set to `false` to restore the pre-Tier-3 advisory behavior. **BC note:** operators with existing `deepReviewPasses: 1` will see warnings/suggestions become blocking unless they explicitly set `deepReviewFindingsAreBlocking: false`. New audit event `pipeline.auto_deep_review_bumped` fires on every bump (payload includes metrics, thresholds, from/to).
...
- **Tier 5 — escalation on consecutive failures** (`pm/actions/promote.ts`): when `promoteReadyIssues` trips the circuit breaker for an issue that does NOT already carry the `needs-design` label, it escalates: adds the `needs-design` label (preserving existing labels), posts a Linear comment summarizing the last failed run's `errorMessage` (truncated 500 chars), invokes the operator-supplied `slackPostAlert(...)` callback, and emits a `pm.escalated_to_needs_design` audit event. Idempotent: subsequent ticks find the label already in place and skip re-escalation (the original `pm.skipped_circuit_breaker` event still fires every tick for observability). The Linear comment is posted even when the `needs-design` label couldn't be resolved (defensive — operator still sees why the agent stopped retrying). `getLastFailureError(db, issueId)` is the DB helper backing the error-message fetch. Tier 5 builds on the existing Tier 4 `needs-design` routing — the same human-gate pipeline picks up escalated issues for design review.
...
- 51 event types (canonical list in `AuditEventTypeSchema` at `packages/core/src/types.ts`): `run.{started,completed,failed,auto_merged,auto_merge_skipped,cancelled}`, `system.halted`, `pm.{approval_requested,approval_resolved,issue_promoted,issue_deprioritized,issue_cancelled,triage_classified,agent_branch_swept,skipped_circuit_breaker,recovered_long_running,escalated_to_needs_design}`, `budget.{alert_fired,run_refused}`, `license.validation_failed`, `config.loaded`, `claude.auth_expired` (BEC-207, AuthMonitor), `dashboard.{manual_action,login,logout,login_denied}`, `policy.{path_blocked,cost_exceeded,override_used,reviewers_requested}`, `release.{fired,skipped,approved,tag_conflict,partial}`, `slack.post_failed`, `qa.{run_triggered,run_completed,gap_issue_filed}`, `review.{fanout_fallback_used,model_low_output_ratio}`, `pipeline.{scratch_files_blocked,typecheck_failed,spec_vs_impl_failed,auto_deep_review_bumped}` (Tiers 1a, 1b, 1c, 3), `service.{installed,uninstalled}` (`ura service install`/`uninstall`, emitted opportunistically from the CLI when the daemon DB exists), `linear.oauth_completed` (`ura self-auth-linear`, emitted opportunistically; payload carries workspaceId/workspaceName, never the token), `tunnel.{started,stopped}` (`ura start --tunnel <mode>`, emitted by the daemon when cloudflared is brought up or shut down), `config.reloaded` (emitted by the daemon when `~/.urateam/config.json` is hot-reloaded; payload carries added/removed/modifiedSafe/modifiedUnsafe arrays + sha256). Tier 1d adds a unit test that fails when this count diverges from the schema.
...
- **Tier 4 — design-doc triage with open-questions routing** (`pm/actions/triage.ts`): The triage prompt is extended to require three new fields beyond the existing classification/AC output — `approachSummary` (3-5 line description of what the implementation will do, sanity-checkable before tokens are spent), `openQuestions` (unanswered questions the operator must resolve before implement is safe), and `antiAcceptanceCriteria` (explicit out-of-scope items so the agent stays narrow). When `openQuestions.length > 0`, the ticket is **forced to the `needs-design` pipeline label** regardless of complexity classification — same routing mechanism as the observer-marker gate above. This gates ambiguous issues for human review before the agent burns implement-stage tokens guessing. The Linear comment posted by triage includes all three new sections when non-empty. `TriageResult` gains optional `approachSummary`, `openQuestions`, `antiAcceptanceCriteria` fields.
