...
- **PR body generation**: `generatePRDescription()` in `pipeline/pr-description.ts` builds markdown for all auto-PRs. `runner.ts` calls it after push, passes the string as the `body` param. Template: `## Summary` → `## Changes` → `## Test plan` → `## Commits` (omitted when empty) → `> Draft PR` callout (omitted when not draft) → `Resolves <issueId>`.
