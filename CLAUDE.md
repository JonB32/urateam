...
- **Session shape (BEC-231)**: derived from on-disk state on every stage entry — `transcriptExists()` is called regardless of in-memory flags. JSONL present → `resume:` (continue). JSONL absent → `sessionId:` (create or retry; the SDK writes the transcript on the first successful message). `agent_session_id` null (flag off) → no session options. Audit event `system.session_volume_warning` fires at boot when the projects dir is missing. The `<previous-stage-context>` block is suppressed in resumed RALPH iterations (the agent already saw it).
