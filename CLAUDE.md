...
`ura bootstrap` — one-command self-hosted onboarding wizard. Flow: preflight (Docker, configurable ports, tools) → optional GitHub App manifest flow → optional Linear webhook registration → generate `.env` + `docker-compose.dogfood.yml` + `Caddyfile` (or cloudflared command) → optional validate via synthetic webhook to `http://localhost:{appPort}/webhooks/linear`. Flags: `--skip-github-app`, `--skip-linear`, `--validate`, `--domain`, `--proxy {caddy|cloudflared}`, `--output-dir`, `--app-port` (default 3000), `--dashboard-port` (default 3001), `--port` (deprecated alias for `--app-port`). All functions accept optional `deps` (`BootstrapDeps`) for injectable `execFile` / `fetch` / `writeFile` / `log` / `openBrowser`.
