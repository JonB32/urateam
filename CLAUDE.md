...
- **Deep review loop convergence (BEC-211)**: `deepReviewPasses` (default 0/disabled) opts into post-review-fix deep-review passes. The loop exits when any of four convergence criteria fires: (1) zero findings remain, (2) accumulated pass count ≥ `maxReviewTurns` (default 15, safety cap independent of `maxDeepReviewPasses`), (3) the same set of files was modified in two consecutive passes (file-oscillation cycle detection), or (4) findings count did not decrease. Exit reason, iteration number, and conflicting file names are logged at INFO level under `"deep review: convergence detected — exiting loop"` with `convergenceReason` and `convergenceDetail` fields. Module: `packages/core/src/pipeline/convergence.ts`; exported as `detectConvergence` from `@urateam/core`.
  - `maxReviewTurns` (integer ≥ 1, default 15) — hard cap on deep-review iterations. Each iteration ≈ 3 agent stages (review-providers + implement + review). With ~25 base turns, keeping `maxReviewTurns ≤ 15` keeps total turns under the quality-observer alert threshold (50). Configure lower (e.g. 5) to save tokens; higher for complex pipelines.
