...
- [X] **T004** [US3] Add `isV2Disabled(env: NodeJS.ProcessEnv = process.env): boolean` export to `packages/core/src/pm/actions/triage-prompt.ts`. Strict equality check on the string `"true"`. Test cases: env unset → false; env `"true"` → true; env `"1"` / `"yes"` / `""` / `"false"` → false; env literal `"TRUE"` (different case) → false. Test file: `packages/core/src/__tests__/triage-v2-env-toggle.test.ts`. — Verification: `pnpm --filter @urateam/core test src/__tests__/triage-v2-env-toggle.test.ts`
