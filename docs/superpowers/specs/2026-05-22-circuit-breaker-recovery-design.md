...
**CLI architecture**: follows the `ura admin` pattern — direct DB access via `createDb()` and direct Linear API via `LINEAR_API_KEY`. NOT the `ura stop`/`ura halt` HTTP pattern, because those modify in-memory daemon state and circuit-breaker reset is pure DB + Linear. No `URATEAM_CLI_TOKEN` required; the operator runs the CLI in the same env that has `DATABASE_URL` + `LINEAR_API_KEY`.
